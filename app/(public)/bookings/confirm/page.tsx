...
import { useAppDispatch } from "@/lib/store"; // FIXED: Uses your type-safe dispatcher hook
...
// Fixed to accurately reflect your Prisma 'Item' model structure
...
export default function BookingConfirmPage() {
  ...
  const dispatch = useAppDispatch(); // Fixed: Using your specific store type dispatcher
  const { getToken } = useAuth();

  const itemId = searchParams?.get("itemId"); // Updated key name from serviceId to match schema
  ...
  useEffect(() => {
    const fetchServiceItem = async () => {
      ...
      try {
        ...
        // Maps structural state down to your Redux layer safely
        dispatch(
          selectService({
            ...
            service: {
              ...
              name: data.name,
              price: data.price,
              duration: data.duration ?? 30, // Defends against null database integers
              image: data.image,
            },
          }),
        );
  ...
  /* ================= FETCH COMPLIANT SCHEDULE ================= */
  useEffect(() => {
    const fetchBusinessSchedule = async () => {
      try {
        const token = await getToken();
        // Scoped to query the central business schedule table layout
  ...
  return (
    <div className="min-h-screen pt-24 px-6 max-w-3xl mx-auto w-full">
      ...
      <div className="mb-8 max-w-xs">
        ...
        <input
          type="date"
          value={date}
          min={new Date().toISOString().split("T")[0]} // Prevents back-dated choices historically
          onChange={(e) => {
            setDate(e.target.value);
            setTime("");
          }}
          className="w-full bg-muted/40 text-foreground border border-border/60 rounded-xl px-4 py-3 outline-hidden focus:border-primary/50 transition-colors"
        />
      </div>
      ...
      {/* CONFIRMATION INITIATION BUTTON */}
      <Button
